...
function appReview() {
	...
	currentImage.setAttribute('src', localStorage.getItem('srcImage'));
...
function userSelectionOfFile() {
	...
	function onSelectFiles(event) {
		...
		if(event.target.files.length > 0) {
			...
			localStorage.setItem('srcImage', currentImage.getAttribute('src'));
...
function clickBurger() {
	...
	localStorage.setItem('<menu>menuMode</menu>', menuInitial);
	// делаем меню в начальном режиме
	menuInitial();
